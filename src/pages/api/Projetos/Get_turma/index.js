...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Busca projetos da turma específica com todas as relações
    ...
    if (projetos.length === 0) {
      return res.status(404).json({ erro: 'Nenhum projeto encontrado para esta turma' });
    }
    ...
    return res.status(500).json({ erro: 'Erro ao buscar projetos da turma' });
  }
}